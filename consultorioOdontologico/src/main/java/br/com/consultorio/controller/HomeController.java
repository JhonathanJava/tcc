...
public class HomeController implements Serializable {
    ...
    private void createLineModels() {
         ...
         lineModel1.getAxes().put(AxisType.X, new CategoryAxis("Mês"));
    ...
    private LineChartModel initPagamentoRecebimento() {
        ...
        Calendar dataAtual = Calendar.getInstance();
 		...
 		//1º dia do mês atual
 		primeiroDia.set(2016, Calendar.JANUARY, 1);
 		System.out.println(primeiroDia.getTime());
 		//Ultimo dia do mês atual
 		ultimoDia.set(2030, Calendar.DECEMBER, 31);
 		System.out.println(ultimoDia.getTime());
 		SimpleDateFormat formatador = new SimpleDateFormat("yyyy-MM-dd");
         
         this.titulosPagar = dao.getSqlListMap("select sum(tit_valor) valor , Month(tit_vencimento) dia from Titulo where tit_tipo = 'D' and tit_vencimento >= '"+formatador.format(primeiroDia.getTime())+"' and tit_vencimento <= '"+formatador.format(ultimoDia.getTime())+"'  group by 2 order by Month(tit_vencimento)");
         ...
         this.titulosReceber = dao.getSqlListMap("select sum(tit_valor) valor , Month(tit_vencimento) dia from Titulo where tit_tipo = 'C' and tit_vencimento >= '"+formatador.format(primeiroDia.getTime())+"' and tit_vencimento <= '"+formatador.format(ultimoDia.getTime())+"'   group by 2 order by Month(tit_vencimento)");
